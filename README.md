...
resource (or the `lumine.updater.uri` system property) and expects as response
...
The `romtype` attribute is the string to be compared with the `org.lumine.build.type` property.  
The `size` attribute is the size of the update expressed in bytes.  
The `url` attribute is the URL of the file to be downloaded.  
The `version` attribute is the string to be compared with the `org.lumine.build.version` property.  
