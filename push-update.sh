...
updates_dir=/data/luminedroid_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have LumineDroid-VERSION-DATE-TYPE-* as format"
...
# Assume LumineDroid-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.luminedroid.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.luminedroid.updater/databases/updates.db" \
